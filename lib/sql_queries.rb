...
"SELECT projects.title, (SUM(pledges.amount) - projects.funding_goal) AS amount_over_funding_goal FROM projects JOIN pledges ON projects.id = pledges.project_id GROUP BY projects.title HAVING amount_over_funding_goal >= 0;"
end

def selects_user_names_and_amounts_of_all_pledges_grouped_by_name_then_orders_them_by_the_amount_and_users_name
"SELECT users.name, SUM(pledges.amount) FROM users JOIN pledges ON user.id = pledges.user_id GROUP BY users.name ORDER BY pledges.amount"
end
